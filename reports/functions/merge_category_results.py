#!/usr/bin/env python3
"""
Scan an outputs directory for folders with "category_model_" in their name,
read each folder's results.json and write a combined JSON array with the
folder name and its results.

Usage:
  python merge_category_results.py \
    --outputs-dir /path/to/outputs \
    --out-file /path/to/combined_category_model_results.json
"""
...
logger = logging.getLogger("merge_category_results")
logging.basicConfig(level=logging.INFO, format="%(levelname)s: %(message)s")


def find_category_dirs(outputs_dir: Path, pattern: str = "category_model_") -> List[Path]:
...
def merge(outputs_dir: Path, out_file: Path, pattern: str = "category_model_") -> None:
    dirs = find_category_dirs(outputs_dir, pattern=pattern)
...
def main() -> None:
    p = argparse.ArgumentParser(description="Merge category_model_* results.json files")
    p.add_argument(
        "--outputs-dir",
        type=Path,
    ...
    p.add_argument(
        "--out-file",
        type=Path,
        default=Path("/dss/dssmcmlfs01/pr74ze/pr74ze-dss-0001/ra95kix2/outputs/combined_category_model_results.json"),
        help="Output combined JSON file path",
    )
    p.add_argument(
        "--pattern",
        type=str,
        default="category_model_",
        help="Directory name pattern to match (default: 'category_model_')",
    )
    args = p.parse_args()
