...
class SkillRetrievalEvaluator(InformationRetrievalEvaluator):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # 1. Get the set of ALL skills that are "findable" in the corpus
        findable_skill_ids = set(corpus.keys())

        # 2. Process Corpus (NEW LOGIC)
        ...
        # We iterate over the corpus dict (c_id -> aliases_str)
        for c_id, aliases_str in corpus.items():
            try:
                ...
                for alias in aliases:
                    ...
                    self.alias_to_skill_map[alias_id] = c_id
                    alias_counter += 1
        ...
        # 3. Process Relevant Docs (NEW FILTERING LOGIC)
        # We filter the original relevant_docs to only include skills
        # that are actually in the corpus (i.e., in findable_skill_ids)
        self.relevant_docs = {}
        for q_id, skill_set in relevant_docs.items():
            # Create a new set containing only skills that are findable
            filtered_skill_set = {c_id for c_id in skill_set if c_id in findable_skill_ids}
            
            # Only add this query if it still has relevant docs
            if filtered_skill_set:
                self.relevant_docs[q_id] = filtered_skill_set
        
        print(f"  ✓ Filtered relevant docs. {len(relevant_docs)} original queries -> {len(self.relevant_docs)} queries with findable relevant docs.")

        # 4. Process Queries (NOW we filter queries)
        # We must only evaluate on queries that have at least one
        # findable relevant doc.
        self.queries_ids = []
        for qid in queries:
        ...
        print(f"  ✓ Final query set size for evaluation: {len(self.queries_ids)}")

        # 5. Copy all other __init__ logic from the parent
