...
# File paths
SIMILARITY_SCORES_PATH = "/dss/dssmcmlfs01/pr74ze/pr74ze-dss-0001/ra95kix2/outputs/decorte_w_desc_2/similarity_scores.json"
JOB_IDS_PATH = "/dss/dsshome1/02/ra95kix2/thesis/skills4cpp/data/title_pairs_desc/decorte_master_2.csv"
...
def calculate_upper_bounds(job_details):
    """Calculate theoretical upper bounds for recall@k metrics"""
    jobs_details_df = pd.DataFrame(job_details)
    
    k_values = [5, 10, 20, 50, 100, 200, 500, 1000]
    upper_bounds = {}
    
    for skill_type in ['all', 'essential', 'optional']:
        upper_bounds[skill_type] = {}
        num_gt_col = f'num_gt_{skill_type}'
        
        if num_gt_col not in jobs_details_df.columns:
            continue
            
        for k in k_values:
            # Upper bound for each job: min(k, num_ground_truth) / num_ground_truth
            jobs_details_df[f'upper_bound_{k}'] = jobs_details_df[num_gt_col].apply(
                lambda x: min(k, x) / x if x > 0 else 0
            )
            upper_bounds[skill_type][k] = jobs_details_df[f'upper_bound_{k}'].mean()
...
def generate_report(results, job_details, top_skills_analysis, isco_df, skill_type_df, jobs_df, occupations_df):
    ...
    upper_bounds = calculate_upper_bounds(job_details)
    ...
    for skill_type in ['all_skills', 'essential_skills', 'optional_skills']:
        ...
        skill_type_key = skill_type.replace('_skills', '')
        ...
        # Recall @ k with Upper Bounds
        report.append("\nRecall @ k (with Upper Bounds):")
        for k in [5, 10, 20, 50, 100, 200, 500, 1000]:
            metric_name = f'R@{k}'
            if metric_name in results[skill_type] and skill_type_key in upper_bounds and k in upper_bounds[skill_type_key]:
                avg_value = np.mean(results[skill_type][metric_name])
                upper_bound = upper_bounds[skill_type_key][k]
                achievement_rate = (avg_value / upper_bound * 100) if upper_bound > 0 else 0
                report.append(f"  R@{k:4d} = {avg_value:.4f}  (Upper Bound: {upper_bound:.4f}, Achievement: {achievement_rate:.1f}%)")
    ...
    # 2b. Upper Bound Analysis
    report.append("\n" + "=" * 100)
    report.append("2b. RECALL UPPER BOUNDS & ACHIEVEMENT RATES")
    report.append("=" * 100)
    report.append("")
    report.append("This section shows the theoretical maximum recall (upper bound) at each k,")
    report.append("and what percentage of that maximum your model achieves.")
    report.append("")
    report.append(f"{'Metric':<10} {'Actual':>10} {'Upper Bound':>12} {'Achievement':>12} {'Note':<50}")
    report.append("-" * 100)
    
    for k in [5, 10, 20, 50, 100, 200, 500, 1000]:
        metric_name = f'R@{k}'
        if metric_name in results['all_skills'] and 'all' in upper_bounds and k in upper_bounds['all']:
            actual = np.mean(results['all_skills'][metric_name])
            upper = upper_bounds['all'][k]
            achievement = (actual / upper * 100) if upper > 0 else 0
            
            # Add contextual note
            if k <= 10:
                note = f"With avg ~57.6 skills, can only find {k} max"
            elif k <= 100:
                note = f"Moderate k relative to avg ground truth"
            else:
                note = f"Large k, approaching full coverage"
            
            report.append(f"R@{k:<7} {actual:>10.4f} {upper:>12.4f} {achievement:>11.1f}% {note:<50}")
    ...
    p5_all = np.mean(results['all_skills']['P@5'])
    r5_all = np.mean(results['all_skills']['R@5'])
    
    essential_in_top10 = top_skills_analysis[10]['essential_pct']
    optional_in_top10 = top_skills_analysis[10]['optional_pct']
    
    # Calculate achievement rates
    r5_upper = upper_bounds['all'][5] if 'all' in upper_bounds and 5 in upper_bounds['all'] else 0
    r5_achievement = (r5_all / r5_upper * 100) if r5_upper > 0 else 0
    
    r100_all = np.mean(results['all_skills']['R@100'])
    r100_upper = upper_bounds['all'][100] if 'all' in upper_bounds and 100 in upper_bounds['all'] else 0
    r100_achievement = (r100_all / r100_upper * 100) if r100_upper > 0 else 0
    
    jobs_details_df = pd.DataFrame(job_details)
    avg_gt_all = jobs_details_df['num_gt_all'].mean()
    ...
    report.append(f"4. Performance Relative to Upper Bounds:")
    report.append(f"   - Average job has {avg_gt_all:.1f} relevant skills")
    report.append(f"   - R@5 upper bound is {r5_upper:.4f} ({r5_upper*100:.1f}%), model achieves {r5_achievement:.1f}% of this")
    report.append(f"     → This means finding {r5_all * avg_gt_all:.1f} out of max possible {r5_upper * avg_gt_all:.1f} skills in top-5")
    report.append(f"   - P@5 of {p5_all:.1%} means ~{p5_all * 5:.1f} out of 5 predictions are correct on average")
    report.append(f"   - R@100 achieves {r100_achievement:.1f}% of upper bound ({r100_all:.4f} / {r100_upper:.4f})")
    report.append(f"   - The model is performing {r100_achievement/100:.1f}x better than random baseline!")
    report.append("")
    ...
    report.append(f"5. Occupation Group Performance:")
    ...
    report.append(f"6. Skill Category Performance:")
...
def main():
    ...
    # Save report to file
    output_path = "/dss/dssmcmlfs01/pr74ze/pr74ze-dss-0001/ra95kix2/outputs/decorte_w_desc_2/evaluation_report.txt"
    ...
    job_details_path = "/dss/dssmcmlfs01/pr74ze/pr74ze-dss-0001/ra95kix2/outputs/decorte_w_desc_2/job_level_metrics.csv"
    job_details_df.to_csv(job_details_path, index=False)
    print(f"Job-level metrics saved to: {job_details_path}")
    
    isco_path = "/dss/dssmcmlfs01/pr74ze/pr74ze-dss-0001/ra95kix2/outputs/decorte_w_desc_2/isco_group_metrics.csv"
    isco_df.to_csv(isco_path, index=False)
    print(f"ISCO group metrics saved to: {isco_path}")
    
    skill_type_path = "/dss/dssmcmlfs01/pr74ze/pr74ze-dss-0001/ra95kix2/outputs/decorte_w_desc_2/skill_category_metrics.csv"
