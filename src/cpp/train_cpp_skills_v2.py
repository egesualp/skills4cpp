...
def process_career_path_batch(
    ...
    device: str = None,
    precomputed_skill_embeddings: Dict[str, Any] = None,
    ...
    if precomputed_skill_embeddings is not None:
        # ------------------------------------------------------------------
        # Optimized GPU path: use precomputed skill embeddings as torch tensors
        # and perform all pooling operations on-device.
        # ------------------------------------------------------------------
        career_job_vectors = [[] for _ in range(batch_size)]

        # Infer embedding device from any example tensor
        ...
        for career_idx in range(batch_size):
            ...
            for skill_info_list in job_skills_list:
                ...
                if job_skill_embeds:
                    # Stack precomputed skill embeddings [n_skills, dim] on-device
                    job_skill_embeds_tensor = torch.stack(job_skill_embeds, dim=0).to(embed_device)
                    ...
                    # Pool skills with IDF weights using torch
                    # Reuse the same logic as NumPy version but in torch
                    if torch.sum(job_idf_weights_tensor) > 0:
                        normalized_weights = job_idf_weights_tensor / torch.sum(job_idf_weights_tensor)
                    else:
                        normalized_weights = torch.ones_like(job_idf_weights_tensor) / len(job_idf_weights_tensor)
    ...
    for career_idx in range(batch_size):
        ...
        if job_vectors:
            if precomputed_skill_embeddings is not None:
                # Torch-based pooling on the same device as skill embeddings
                n_jobs = len(job_vectors)
                device_for_jobs = job_vectors[0].device
                if alpha == 0:
                    weights = torch.ones(n_jobs, device=device_for_jobs) / n_jobs
                else:
                    positions = torch.arange(n_jobs, dtype=torch.float32, device=device_for_jobs)
                ...
                # For training, downstream code expects something array-like;
                # we keep torch tensors here and handle them in the train loop.
                career_path_embeddings.append(pooled_career)
            else:
                # Original NumPy-based pooling path
            ...
            if precomputed_skill_embeddings is not None:
                # Create zero vector on the same device as embeddings
...
def precompute_skill_embeddings(
    ...
    # Encode all skills once as torch tensors on the specified device
    skill_embeddings = encoder.encode(
        skill_texts,
    ...
    # SentenceTransformer.encode with convert_to_numpy=False returns a single
    # tensor of shape [n_skills, dim]. Keep embeddings on-device to avoid
    # repeated host<->device transfers during training.
    uri_to_emb = {}
...
def evaluate_ranking(
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader, desc="Evaluating", leave=False):
            # Process batch to get career path embeddings.
            # For evaluation we use the original NumPy/encode path to keep
            # everything on CPU and avoid coupling to the training-time
            # precomputed embedding format. When precomputed_skill_embeddings
            # is provided, we reuse the same pooled representations as in
            # training (torch tensors on-device) and convert to NumPy here.
            ...
            if precomputed_skill_embeddings is not None:
                # career_embeds is a list of torch.Tensors on the embedding device
                pred_embeddings.extend(
                    [emb.detach().cpu().numpy() for emb in career_embeds]
                )
            else:
                # career_embeds is a list of NumPy arrays
...
def compute_all_target_embeddings(
    ...
    """
    Encode all unique target occupation texts with the *current* model.
    This is called before each validation / test evaluation so that
    ranking metrics always reflect the latest model weights.
    """
    logger.info(
        f"  Encoding {len(all_target_texts)} unique target occupations for evaluation..."
    )
...
def manual_train_loop(
    ...
    for epoch in range(args.num_epochs):
        ...
        for batch in tqdm(train_loader, desc=f"Training Epoch {epoch+1}"):
            ...
            # First, process the batch to obtain fixed career-path embeddings
            career_embeds, target_texts = process_career_path_batch(
                batch,
                skill_desc_map,
                model,
            ...
            if precomputed_skill_embeddings is not None:
                # Torch-based path: process_career_path_batch already returned
                # a list of torch tensors on the correct device.
                career_embeds_tensor = torch.stack(career_embeds, dim=0)
            else:
                # Fallback NumPy path
        ...
        # Recompute target embeddings with the *current* model weights so that
        # ranking metrics reflect the latest state of the encoder.
        current_target_embeddings = compute_all_target_embeddings(
            model,
            all_target_texts,
            device,
            batch_size=256,
...
def main():
    ...
    # Recompute target embeddings with the final (best) model for test evaluation
    all_target_embeddings = compute_all_target_embeddings(
        model,
        all_target_texts,
        device,
        batch_size=256,
