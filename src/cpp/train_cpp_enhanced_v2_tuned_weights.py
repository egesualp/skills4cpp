...
class LearnablePoolingLayer(nn.Module):
    """
    Learnable weighted pooling for skill embeddings.
    
    Uses index-based skill embeddings with a frozen lookup table to minimize memory.
    Computes: W = (Scores^α) ⊙ (IDF^β) ⊙ (Temporal^γ) ⊙ Mask
    Then: V_path = Σ(W_i · E_i) / Σ(W_i)
    
    Args:
        skill_embedding_matrix: Frozen skill embeddings [num_skills, embed_dim]
        initial_alpha: Initial value for confidence score exponent (default: 1.0)
        initial_beta: Initial value for IDF exponent (default: 1.0)
        initial_gamma: Initial value for temporal exponent (default: 0.0)
    """
    
    def __init__(self, skill_embedding_matrix, initial_alpha=1.0, initial_beta=1.0, initial_gamma=0.0):
        super().__init__()
        
        # Frozen skill embedding lookup table
        num_skills, embed_dim = skill_embedding_matrix.shape
        self.skill_embeddings = nn.Embedding.from_pretrained(
            torch.from_numpy(skill_embedding_matrix).float(),
            freeze=True,
            padding_idx=0  # Reserve index 0 for padding
        )
        self.embed_dim = embed_dim
        
        # Initialize learnable parameters in log-space to ensure positivity
        # Using log-space: alpha = exp(log_alpha) ensures alpha > 0
        ...
        # Add small epsilon for gamma=0 case to avoid log(0)
        self.log_gamma = nn.Parameter(torch.log(torch.tensor(max(initial_gamma, 1e-8))))
    
    def forward(self, skill_indices, scores, idf_values, temporal_indices, mask):
        """
        Apply learnable weighted pooling over skills.
        
        Args:
            skill_indices: [batch_size, max_skills] - Integer indices into skill embedding table
            scores: [batch_size, max_skills] - Confidence scores for each skill
            idf_values: [batch_size, max_skills] - IDF scores for each skill
            temporal_indices: [batch_size, max_skills] - Job-level position indices (0-indexed)
            mask: [batch_size, max_skills] - Binary mask (1=real skill, 0=padding)
        
        Returns:
            pooled: [batch_size, embed_dim] - Weighted pooled skill embeddings
        """
        # Look up skill embeddings from frozen table
        # [batch_size, max_skills, embed_dim]
        skill_embeddings = self.skill_embeddings(skill_indices)
        
        # Ensure parameters stay positive using exp transform
        alpha = torch.exp(self.log_alpha)
        beta = torch.exp(self.log_beta)
        gamma = torch.exp(self.log_gamma)
        
        # Compute weights: (scores^α) * (idf^β) * ((temporal + 1)^γ)
        # Add 1.0 to temporal_indices so first job (index 0) has weight 1^γ = 1
        weights = (scores ** alpha) * (idf_values ** beta) * ((temporal_indices + 1.0) ** gamma)
        
        # Apply mask to zero out padding tokens (MANDATORY)
        ...
        weights_normalized = weights / weights_sum  # [batch_size, max_skills]
        
        # Weighted sum: [batch_size, max_skills, 1] * [batch_size, max_skills, embed_dim]
        # -> sum over max_skills dim -> [batch_size, embed_dim]
        pooled = (weights_normalized.unsqueeze(-1) * skill_embeddings).sum(dim=1)
        
        return pooled
    
    def get_current_params(self):
        """Return current values of alpha, beta, gamma (for logging)."""
        return {
            'alpha': torch.exp(self.log_alpha).item(),
            'beta': torch.exp(self.log_beta).item(),
            'gamma': torch.exp(self.log_gamma).item()
        }
...
class MultiModalCPPModel(nn.Module):
    ...
    def __init__(self, text_dim, skill_text_dim, structured_dim, 
                 ...
                 skill_embedding_matrix=None, use_learnable_pooling=False,
                 initial_alpha=1.0, initial_beta=1.0, initial_gamma=0.0):
        super().__init__()
        ...
        if use_skill:
            if use_learnable_pooling and skill_embedding_matrix is not None:
                # Add learnable pooling layer
                self.skill_pooling = LearnablePoolingLayer(
                    skill_embedding_matrix,
                    initial_alpha=initial_alpha,
                    initial_beta=initial_beta,
                    initial_gamma=initial_gamma
                )
                # Skill encoder takes pooled embedding as input
                self.skill_encoder = self._build_encoder(
                    self.skill_pooling.embed_dim, hidden_dim, n_layers, dropout
                )
            else:
                # Use pre-pooled embeddings (existing path)
    ...
    def forward(self, batch):
        ...
        if self.use_skill:
            if self.use_learnable_pooling and hasattr(self, 'skill_pooling'):
                # Apply learnable pooling first
                h_skill_pooled = self.skill_pooling(
                    batch['skill_indices'],
                    batch['skill_scores'],
                    batch['skill_idf'],
                    batch['skill_temporal'],
                    batch['skill_mask']
                )
                h_skill = self.skill_encoder(h_skill_pooled)
            else:
                # Use pre-pooled embeddings (existing path)
...
class SimpleConcatModel(nn.Module):
    ...
    def __init__(self, input_dim, output_dim, n_layers, hidden_dim, dropout,
        ...
        # Add learnable pooling for skills if enabled
        if use_skill and use_learnable_pooling and skill_embedding_matrix is not None:
            self.skill_pooling = LearnablePoolingLayer(
                skill_embedding_matrix,
                initial_alpha=initial_alpha,
                initial_beta=initial_beta,
    ...
    def forward(self, batch):
        ...
        if self.use_skill:
            if self.use_learnable_pooling and hasattr(self, 'skill_pooling'):
                ...
                features.append(h_skill_pooled)
            else:
                # Use pre-pooled embeddings
...
def objective(trial, train_loader, val_sample_loader, Y_target_all, args,
    ...
    if args.use_advanced:
        ...
        model = MultiModalCPPModel(
            ...
            skill_embedding_matrix=skill_embedding_matrix,
            use_learnable_pooling=args.use_learnable_pooling,
    ...
    # Separate pooling parameters from model parameters
    pooling_params = []
    model_params = []
    
    if hasattr(model, 'skill_pooling'):
        pooling_params = [
            model.skill_pooling.log_alpha,
            model.skill_pooling.log_beta,
            model.skill_pooling.log_gamma
        ]
        pooling_param_ids = {id(p) for p in pooling_params}
        model_params = [p for p in model.parameters() if id(p) not in pooling_param_ids]
        logger.info(f"  > Using learnable pooling: {len(pooling_params)} pooling params")
    else:
    ...
    if args.optimizer == "sgd":
        ...
        if pooling_params:
            optimizer = optim.SGD([
                ...
                {'params': pooling_params, 'lr': lr * args.pooling_lr_multiplier}
            ], momentum=momentum, weight_decay=weight_decay, nesterov=nesterov)
            logger.info(f"  > Pooling LR: {lr * args.pooling_lr_multiplier:.6f}")
        else:
            optimizer = optim.SGD(model_params, lr=lr, momentum=momentum, 
                                 weight_decay=weight_decay, nesterov=nesterov)
        logger.info(f"  > SGD: lr={lr:.6f}, momentum={momentum:.4f}, wd={weight_decay:.6f}")
    else:  # adam
        weight_decay = trial.suggest_float("weight_decay", 1e-6, 1e-2, log=True)
        ...
        logger.info(f"  > Adam: lr={lr:.6f}, weight_decay={weight_decay:.6f}")
    ...
    for epoch in range(args.max_epochs):
        ...
        # Log pooling parameters
        if hasattr(model, 'skill_pooling'):
            alpha = torch.exp(model.skill_pooling.log_alpha).item()
            beta = torch.exp(model.skill_pooling.log_beta).item()
            gamma = torch.exp(model.skill_pooling.log_gamma).item()
            logger.info(f"  📊 Pooling params: α={alpha:.4f}, β={beta:.4f}, γ={gamma:.4f}")
            
            log_metrics['pooling_alpha'] = alpha
            log_metrics['pooling_beta'] = beta
            log_metrics['pooling_gamma'] = gamma
...
def parse_args():
    ...
    # Learnable pooling configuration
    parser.add_argument("--use_learnable_pooling", action='store_true',
                       help="Use learnable α, β, γ parameters optimized via backpropagation (requires skill embeddings)")
    parser.add_argument("--pooling_lr_multiplier", type=float, default=0.1,
                       help="Learning rate multiplier for pooling parameters α, β, γ (default: 0.1x base LR)")
    parser.add_argument("--initial_alpha", type=float, default=1.0,
                       help="Initial value for α (confidence score exponent)")
    parser.add_argument("--initial_beta", type=float, default=1.0,
                       help="Initial value for β (IDF exponent)")
    parser.add_argument("--initial_gamma", type=float, default=0.0,
                       help="Initial value for γ (temporal/job position exponent)")
    parser.add_argument("--max_skills_per_path", type=int, default=400,
                       help="Maximum number of skills per career path (for learnable pooling)")
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Apply Min-Max normalization to skill scores per job (scales to [0, 1]).",
    )
...
def main():
    ...
    job_skill_map, esco_skill_text_map, skill_properties_map = load_job_skill_data_by_id(
        ...
        min_max_normalize=args.min_max_normalize_skills
    )
    ...
    # Prepare skill embeddings for learnable pooling
    skill_embedding_matrix = None
    skill_uri_to_idx = None
    
    if args.use_learnable_pooling:
        logger.info("🔧 Preparing skill embeddings for learnable pooling...")
        from src.cpp.data_loaders_tuned import build_skill_embedding_matrix_and_mapping
        
        # Build embedding matrix and URI-to-index mapping
        skill_embedding_matrix, skill_uri_to_idx = build_skill_embedding_matrix_and_mapping(
            precomputed_skill_embedding_map
        )
        logger.info(f"  ✓ Skill embedding matrix: {skill_embedding_matrix.shape}")
    
    # Create dataset (conditional on learnable pooling)
    if args.use_learnable_pooling:
        from src.cpp.cpp_dataset_tuned import SkillsRawDataset, collate_skills_raw_batch
        
        logger.info("📦 Creating SkillsRawDataset for learnable pooling...")
        train_dataset = SkillsRawDataset(
            data_pairs=train_pairs,
            job_ids_list=train_job_ids,
            encoder=encoder_text,
            Y_target_dict=Y_target_dict,
            job_skill_map=job_skill_map,
            skill_uri_to_idx=skill_uri_to_idx,
            max_skills_per_path=args.max_skills_per_path,
            include_text=args.use_text_history,
            include_structured=args.use_structured,
            pre_h_text=train_h_text if args.use_text_history else None,
            skill_properties_map=skill_properties_map if args.use_structured else None,
            all_vocabs=all_vocabs if args.use_structured else None,
            device=device,
            pin_embeddings_to_gpu=args.pin_embeddings_to_gpu
        )
        
        train_collate_fn = collate_skills_raw_batch
    else:
        # Use existing CareerPathDataset (pre-pooled embeddings)
        logger.info("📦 Creating CareerPathDataset with pre-pooled skill embeddings...")
        ...
        train_collate_fn = collate_career_path_batch

    # Use the same dataset type for val/test as for training
    # When learnable pooling is enabled, we need SkillsRawDataset for all splits
    if args.use_learnable_pooling:
        from src.cpp.cpp_dataset_tuned import SkillsRawDataset, collate_skills_raw_batch
        
        val_dataset = SkillsRawDataset(
            data_pairs=val_pairs,
            job_ids_list=val_job_ids,
            encoder=encoder_text,
            Y_target_dict=Y_target_dict,
            job_skill_map=job_skill_map,
            skill_uri_to_idx=skill_uri_to_idx,
            ...
            include_text=args.use_text_history,
            include_structured=args.use_structured,
            pre_h_text=val_h_text if args.use_text_history else None,
            skill_properties_map=skill_properties_map if args.use_structured else None,
            all_vocabs=all_vocabs if args.use_structured else None,
            device=device,
            pin_embeddings_to_gpu=args.pin_embeddings_to_gpu
        ...
        test_dataset = SkillsRawDataset(
            ...
            include_text=args.use_text_history,
            include_structured=args.use_structured,
            pre_h_text=test_h_text if args.use_text_history else None,
            skill_properties_map=skill_properties_map if args.use_structured else None,
            all_vocabs=all_vocabs if args.use_structured else None,
            device=device,
            pin_embeddings_to_gpu=args.pin_embeddings_to_gpu
        ...
        val_collate_fn = collate_skills_raw_batch
    else:
        ...
        val_collate_fn = collate_career_path_batch
    
    test_clean_loader = None
    if args.eval_clean_test:
        if args.use_learnable_pooling:
            test_clean_dataset = SkillsRawDataset(
                data_pairs=test_clean_pairs,
                job_ids_list=test_clean_job_ids,
                encoder=encoder_text,
                Y_target_dict=Y_target_dict,
                job_skill_map=job_skill_map,
                skill_uri_to_idx=skill_uri_to_idx,
                ...
                include_text=args.use_text_history,
                include_structured=args.use_structured,
                pre_h_text=test_clean_h_text if args.use_text_history else None,
                skill_properties_map=skill_properties_map if args.use_structured else None,
                all_vocabs=all_vocabs if args.use_structured else None,
                device=device,
                pin_embeddings_to_gpu=args.pin_embeddings_to_gpu
    ...
    train_loader = DataLoader(
        ...
        num_workers=args.num_workers, collate_fn=train_collate_fn,
    ...
    val_loader = DataLoader(
        ...
        num_workers=args.num_workers, collate_fn=val_collate_fn,
        pin_memory=use_pin_memory,
        persistent_workers=(args.num_workers > 0),
        prefetch_factor=2 if args.num_workers > 0 else None
    )
    ...
    if args.optuna:
        ...
        if hasattr(final_model, 'skill_pooling'):
            pooling_params = [
                final_model.skill_pooling.log_alpha,
                final_model.skill_pooling.log_beta,
                final_model.skill_pooling.log_gamma
            ]
            pooling_param_ids = {id(p) for p in pooling_params}
            model_params = [p for p in final_model.parameters() if id(p) not in pooling_param_ids]
            logger.info(f"  > Using learnable pooling: {len(pooling_params)} pooling params")
        else:
            model_params = list(final_model.parameters())
        ...
        for epoch in trange(args.max_epochs, desc="Epochs", unit="epoch"):
            ...
            # Log pooling parameters if available
            if hasattr(final_model, 'skill_pooling'):
                alpha = torch.exp(final_model.skill_pooling.log_alpha).item()
                beta = torch.exp(final_model.skill_pooling.log_beta).item()
                gamma = torch.exp(final_model.skill_pooling.log_gamma).item()
                logger.info(f"  📊 Pooling params: α={alpha:.4f}, β={beta:.4f}, γ={gamma:.4f}")
                
                if WANDB_AVAILABLE and args.use_wandb:
                    wandb.log({
                        'final_train_loss': train_loss,
                        'final_val_mrr': val_mrr,
                        'final_val_loss': val_loss if val_loss is not None else 0,
                        'final_pooling_alpha': alpha,
                        'final_pooling_beta': beta,
                        'final_pooling_gamma': gamma
                    }, step=epoch)
    ...
    if not skip_final_training:
        ...
        combined_job_ids = train_job_ids + val_job_ids  # Needed for learnable pooling
        ...
        # Use same dataset type as training (learnable pooling vs pre-pooled)
        if args.use_learnable_pooling:
            from src.cpp.cpp_dataset_tuned import SkillsRawDataset, collate_skills_raw_batch
            
            combined_dataset = SkillsRawDataset(
                data_pairs=combined_pairs,
                job_ids_list=combined_job_ids,
                encoder=encoder_text,
                Y_target_dict=Y_target_dict,
                job_skill_map=job_skill_map,
                skill_uri_to_idx=skill_uri_to_idx,
                ...
                include_text=args.use_text_history,
                include_structured=args.use_structured,
                pre_h_text=combined_h_text if args.use_text_history else None,
                skill_properties_map=skill_properties_map if args.use_structured else None,
                all_vocabs=all_vocabs if args.use_structured else None,
                device=device,
                pin_embeddings_to_gpu=args.pin_embeddings_to_gpu
            )
            combined_collate_fn = collate_skills_raw_batch
        else:
            ...
            combined_collate_fn = collate_career_path_batch
        ...
        combined_loader = DataLoader(
            ...
            num_workers=final_num_workers, collate_fn=combined_collate_fn,
            pin_memory=use_pin_memory,
