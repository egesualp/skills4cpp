...
# ============================================================================
# LEARNABLE POOLING UTILITIES
# ============================================================================

def build_skill_embedding_matrix_and_mapping(
    skill_embedding_dict: Dict[str, np.ndarray],
    padding_idx: int = 0
) -> Tuple[np.ndarray, Dict[str, int]]:
    """
    Build skill embedding matrix and URI-to-index mapping for nn.Embedding.
    
    This function prepares skill embeddings for use with PyTorch's nn.Embedding layer
    in the learnable pooling setup. Index 0 is reserved for padding.
    
    Args:
        skill_embedding_dict: Dict mapping skill URIs to embeddings [embed_dim]
        padding_idx: Reserved index for padding (default: 0, will be zero vector)
    
    Returns:
        Tuple of:
        - skill_embedding_matrix: [num_skills+1, embed_dim] numpy array with padding at index 0
        - skill_uri_to_idx: Dict mapping skill URI -> integer index (1 to num_skills)
    """
    if not skill_embedding_dict:
        raise ValueError("skill_embedding_dict must not be empty")
    
    # Get embedding dimension from first embedding
    first_emb = next(iter(skill_embedding_dict.values()))
    embed_dim = first_emb.shape[0]
    
    # Reserve index 0 for padding
    num_skills = len(skill_embedding_dict)
    skill_embedding_matrix = np.zeros((num_skills + 1, embed_dim), dtype=np.float32)
    skill_uri_to_idx = {}
    
    # Start indexing from 1 (0 reserved for padding)
    for idx, (skill_uri, embedding) in enumerate(skill_embedding_dict.items(), start=1):
        skill_embedding_matrix[idx] = embedding.astype(np.float32)
        skill_uri_to_idx[skill_uri] = idx
    
    logger.info(f"  ✓ Built skill embedding matrix: {skill_embedding_matrix.shape}")
    logger.info(f"  ✓ Created URI-to-index mapping for {num_skills} skills (index 0 reserved for padding)")
    
    return skill_embedding_matrix, skill_uri_to_idx
