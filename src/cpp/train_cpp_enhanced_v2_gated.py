...
class GatedMultiModalCPPModel(nn.Module):
    def __init__(self, text_dim, skill_text_dim, structured_dim, 
                 hidden_dim, n_layers, dropout, output_dim,
                 use_text=True, use_skill=True, use_struct=True):
        super().__init__()
        ...
        # Encoders (Senin orijinal yapın)
        if use_text: self.text_encoder = self._build_encoder(text_dim, hidden_dim, n_layers, dropout)
        if use_skill: self.skill_encoder = self._build_encoder(skill_text_dim, hidden_dim, n_layers, dropout)
        if use_struct: self.struct_encoder = self._build_encoder(structured_dim, hidden_dim, n_layers, dropout)
        
        # --- GATED FUSION EKLEMESİ ---
        # Bu ağaç, her batch için modalitelerin ağırlığını (0-1 arası) hesaplar.
        self.gate_network = nn.Sequential(
            nn.Linear(hidden_dim * n_modalities, n_modalities),
            nn.Softmax(dim=1) # Toplam ağırlığı 1'e sabitler (Dikkat: Sigmoid de olabilir)
        )
        # -----------------------------
    ...
    def forward(self, batch):
        ...
        # 1. Her modaliteyi encode et (Boyut: [Batch, hidden_dim])
        if self.use_text: encoded_modalities.append(self.text_encoder(batch['h_text']))
        if self.use_skill: encoded_modalities.append(self.skill_encoder(batch['h_skill_text']))
        if self.use_struct:
            s_keys = [k for k in batch.keys() if k.startswith('h_structured_')]
            encoded_modalities.append(self.struct_encoder(torch.cat([batch[k] for k in s_keys], dim=1)))
        
        # 2. Kapı (Gate) değerlerini hesapla
        # Önce hepsini concat ediyoruz ki kapı "tüm resme" bakıp karar versin
        all_concat = torch.cat(encoded_modalities, dim=1) 
        gate_weights = self.gate_network(all_concat) # Boyut: [Batch, n_modalities]
        
        # 3. Kapıları uygula (Dynamic Weighting)
        gated_features = []
        for i, h_mod in enumerate(encoded_modalities):
            # gate_weights[:, i] o modalitenin o örnek için ağırlığıdır (skaler)
            gated_features.append(h_mod * gate_weights[:, [i]])
...
def main():
    ...
    if n_active_modalities >= 2 and not args.use_advanced:
        ...
        logger.info("   To use GatedMultiModalCPPModel (late fusion), add --use_advanced flag.\n")
    ...
    if args.optuna:
        ...
        if args.use_advanced:
            final_model = GatedMultiModalCPPModel(
                text_dim=text_dim,
                skill_text_dim=skill_text_dim,
                structured_dim=structured_dim,
                hidden_dim=hidden_dim,
    ...
    results_data = {
        ...
        'architecture': 'GatedMultiModal' if args.use_advanced else 'SimpleConcat',
