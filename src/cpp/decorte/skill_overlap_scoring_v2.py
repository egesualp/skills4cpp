...
def load_career_skill_map(skill_scores_file: str, top_k_skills: Optional[int] = None) -> Dict[str, Set[str]]:
    ...
    # We need to handle two potential formats for the skill list items:
    # 1. Dict format: {'skill_uri': '...', 'score': ...} (Original)
    # 2. List/Tuple format: ['uri', score] (New from fused_scorer)
    ...
    for job_id, raw_skill_list in scores_dict.items():
        ...
        # Add URIs to set
        for uri, _ in skill_list_filtered:
            skill_uris.add(uri)
